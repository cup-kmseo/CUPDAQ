#include "ELog.hh"
#include "NKFADC500.hh"

namespace {
constexpr uint16_t kVENDOR_ID = 0x0547;
constexpr uint16_t kPRODUCT_ID = 0x1502;

// Register Addresses
constexpr uint32_t kAddr_BCOUNT = 0x30000000u;
constexpr uint32_t kAddr_DATA = 0x40000000u;

constexpr uint32_t kBlockSize = 256u;
} // namespace

NKFADC500::NKFADC500()
  : _sid(0),
    _usb(kVENDOR_ID, kPRODUCT_ID, 0)
{
}

NKFADC500::NKFADC500(int sid)
  : _sid(sid),
    _usb(kVENDOR_ID, kPRODUCT_ID, sid)
{
}

NKFADC500::~NKFADC500() { Close(); }

void NKFADC500::SetSID(int sid)
{
  _sid = sid;
  _usb.Set(kVENDOR_ID, kPRODUCT_ID, _sid);
}

int NKFADC500::Open()
{
  int status = _usb.Open();
  if (status < 0) {
    ERROR("NKFADC500: Failed to open device (sid=%d)", _sid);
    return status;
  }

  status = _usb.ClaimInterface(0);
  if (status < 0) {
    ERROR("NKFADC500: Failed to claim interface 0 (sid=%d)", _sid);
    _usb.Close();
    return status;
  }

  return 0;
}

void NKFADC500::Close()
{
  _usb.ReleaseInterface(0);
  _usb.Close();
}

uint32_t NKFADC500::ReadBCount() const { return _usb.ReadReg(kAddr_BCOUNT); }

int NKFADC500::ReadData(uint32_t bcount, unsigned char * data, unsigned int timeout) const
{
  if (bcount == 0 || data == nullptr) {
    ERROR("NKFADC500: Invalid ReadData arguments (bcount=%u, data=%p)", bcount,
          static_cast<void *>(data));
    return -1;
  }

  // Calculate total bytes: bcount * 256 bytes per block
  uint32_t total_bytes = bcount * kBlockSize;

  return _usb.Read(total_bytes, kAddr_DATA, data, timeout);
}